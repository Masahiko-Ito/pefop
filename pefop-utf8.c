 ...
 *
 *    $ indent -i8 -kr -l256 pefop-utf8.c
 *
 */
...
const char *Amsg = "pefop-utf8 version 0.4.5 by Masahiko Ito.\nToggleKey=^O\n";
...
void update_candlist()
{
	...
	tputs(Ts, 1, put1ch);
	tputs(Ce, 1, put1ch);
	write(Rfd, (char *) Mode_name[Mode], strlen((char *) Mode_name[Mode]));
	...
	for (i = Page[Curpage]
		...
		if (i == Curcand && rev) {
			tputs(So, 1, put1ch);
			write_utf8(Rfd, (char *) candbuf, strlen((char *) candbuf));
			tputs(Se, 1, put1ch);
		} else {
			write_utf8(Rfd, (char *) candbuf, strlen((char *) candbuf));
		}
